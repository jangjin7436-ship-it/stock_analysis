...
# 페이지 설정 (전체 화면 사용)
st.set_page_config(page_title="AI Sniper Backtest", layout="wide")
...
# =========================================================
# 3. 백테스트 데이터 준비 및 엔진 (핵심 수정 부분 포함)
# =========================================================

def prepare_stock_data(ticker_info, start_date):
    """
    개별 종목 데이터를 준비하여 백테스트 엔진에 전달하는 구조체 생성
    """
    ...
    try:
        ...
        # ★ [수정됨] MACD_Hist를 반환 컬럼에 추가 (동점자 처리용)
        return df[['Close_Calc', 'AI_Score', 'STD20', 'Ticker', 'Name', 'MACD_Hist']]
...
def run_portfolio_backtest(targets, start_date, initial_capital, strategy_mode,
    ...
    for date in sorted_dates:
        ...
        for ticker in sorted(portfolio.keys()):
            ...
            # 3) 수익 및 손절 관리 (AI Sniper 전략 기준)
            if not should_sell:
                # 손절
                if profit_pct <= -3.0: # 스나이퍼 모드: -3% 칼손절
                    should_sell = True
                    sell_reason = "⚡ 손절(-3%)"
                
                # 익절 (트레일링 스탑)
                elif info['max_price'] > info['avg_price'] * 1.05: # +5% 도달 후
                    if curr_price_krw < info['avg_price'] * 1.01: 
                        should_sell = True
                        sell_reason = "🛡️ 수익반납방어"
                    elif drawdown_from_peak <= -0.03: # 고점 대비 -3%
                        should_sell = True
                        sell_reason = "📉 트레일링(-3%)"
        ...
        if len(portfolio) < max_slots:
            ...
            # ★ [정렬 로직 변경] 
            # 1순위: Score (높은 순)
            # 2순위: MACD_Hist (높은 순 -> 상승 가속도)
            candidates.sort(key=lambda x: (x['score'], x['macd_hist']), reverse=True)
            ...
            for target in buy_targets:
                ...
                # 자금 배분 (복리/단리 적용은 balance 자체가 변하므로 자연스럽게 적용됨)
...
# 탭 구성 (단독 실행을 위해 탭 하나만 생성)
tab_backtest = st.tabs(["📊 전체 백테스트 시뮬레이션"])[0]

with tab_backtest:
    st.markdown("### 🧪 포트폴리오 유니버스 백테스트")
    st.caption("AI 전략 시뮬레이터 Final Ver. (MACD 가속도 우선 매수 반영)")
    ...
    with r1_c1:
        ...
        max_hold_days = st.slider("⏱️ 타임 컷 (일)", 0, 60, 14, help="매수 후 N일 지나면 강제 매도 (0=사용안함)")
    with r1_c2:
